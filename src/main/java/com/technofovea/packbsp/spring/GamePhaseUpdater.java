...
public class GamePhaseUpdater extends AbstractPackbspComponent implements ApplicationContextAware, InitializingBean {
    ...
    public void updatePhase(DetectedGame chosenGame, Profile chosenProfile) throws PhaseFailedException {
        try {
            ...
            final ApplicationContext child = initChildContext(chosenProfile);
            final GraphStarterChoice starter; 
            try {
                starter = child.getBean("graph-starter", GraphStarterChoice.class);
            ...
            profileState.setGraphStarter(starter);
            //Modify target state
        }
